...
AllocatedImage::AllocatedImage(ImageType type,VkFormat format,VkImageUsageFlags usageFlags,VkExtent2D extent,
                               int mipLevel,int layer):
                               imageFormat(format), imageExtent(VkExtent3D {extent.width,extent.height, 1}),
                               mipLevels(mipLevel),layer(layer)
{
...
void AllocatedImage::LoadData(std::shared_ptr<Res::ResTexture> resTexture)
{
    ...
    region.imageSubresource.layerCount = resTexture->layer;
